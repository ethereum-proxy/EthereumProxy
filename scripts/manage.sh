...
install(){    
    if [ -d "/root/Ethereum_proxy" ]; then
        echo -e "檢測到您已安裝EthereumProxy，請勿重複安裝，如您確認您未安裝請使用rm -rf /root/Ethereum_proxy指令" && exit 1
    fi
    if screen -list | grep -q "Ethereum_proxy"; then
        echo -e "檢測到您的EthereumProxy已啟動，請勿重複安裝" && exit 1
    fi
    ...
    sleep 2s
    echo "EthereumProxy V1.0 已經安裝到/root/Ethereum_proxy"
...
update(){
    ...
    sleep 2s
    echo "EthereumProxy 已經更新至 V1.0 版本並啟動"
    cat /root/Ethereum_proxy/pwd.txt
    echo ""
